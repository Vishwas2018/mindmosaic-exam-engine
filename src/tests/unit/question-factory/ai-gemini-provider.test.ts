...
function geminiContentResponse(text: string): unknown {
...
describe("GeminiProvider", () => {
  ...
  it("sends the correct Gemini generateContent request shape", async () => {
    ...
    const body = JSON.parse(init.body as string) as { contents: readonly { role: string; parts: readonly { text: string }[] }[] };
    expect(body.contents).toHaveLength(1);
    expect(body.contents[0].role).toBe("user");
    expect(JSON.parse(body.contents[0].parts[0].text)).toEqual(MINIMAL_GENERATION_PACK);
  });
