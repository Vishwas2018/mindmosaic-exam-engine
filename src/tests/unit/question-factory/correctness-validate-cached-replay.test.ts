...
function buildPassedReplayFixture(): {
  ...
  const result = verifyCandidateCorrectness(candidateBeforePass, { verifiedAt: VERIFIED_AT, structuralEvidence });
  ...
  return { candidate, structuralReport, correctnessReport };
}

describe("validateCachedCorrectnessReplay — valid replay", () => {
  it("accepts a genuinely unmodified passed candidate with matching reports", () => {
