...
export const assignmentTargetModeSchema = z.enum(["class", "group", "individual"]);
export type AssignmentTargetMode = z.infer<typeof assignmentTargetModeSchema>;

export const assignmentContentModeSchema = z.enum([
  "practice_set",
  "skill_based",
  "blueprint_based",
]);
export type AssignmentContentMode = z.infer<typeof assignmentContentModeSchema>;

export const assignmentDifficultySchema = z.enum(["foundation", "standard", "challenge"]);
...
export const assignmentConfigSchema = examSelectionConfigSchema.extend({
  ...
  /**
   * Wizard metadata below is additive and presentation/targeting-only —
   * the underlying question selection still runs on yearLevel/examStyle/
   * subject/questionCount above. Stored in the same `config` jsonb column,
   * so no migration is needed to grow this shape further.
   */
  targetMode: assignmentTargetModeSchema.default("class"),
  contentMode: assignmentContentModeSchema.default("practice_set"),
  difficulty: assignmentDifficultySchema.default("standard"),
