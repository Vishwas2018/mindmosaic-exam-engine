    ...
    <fieldset className="space-y-4" disabled={disabled} aria-describedby={instructionsId}>
      ...
      <ul className="grid gap-3">
        {interaction.labels.map((label) => {
          ...
          const chosen = current[label.id];
          const state = reveal ? resolvePairState(reveal, label.id, chosen) : undefined;
          const signal = state ? stateSignal(state) : undefined;
          const correctTargetId =
            reveal && state !== "correct" && reveal.answerKey.kind === "matching"
              ? reveal.answerKey.pairs.find((pair) => pair.sourceId === label.id)?.targetId
              : undefined;
          const correctTargetLabel = correctTargetId
            ? interaction.targets.find((target) => target.id === correctTargetId)?.label
            : undefined;
          return (
            <li
              key={label.id}
            ...
            >
              ...
              {signal ? (
                <span className={`flex items-center gap-1.5 text-[12.5px] font-semibold sm:col-span-2 ${signal.textClass}`}>
                  ...
                  {signal.label}
                  {correctTargetLabel ? `: ${correctTargetLabel}` : ""}
                </span>
              ) : null}
            </li>
          );
