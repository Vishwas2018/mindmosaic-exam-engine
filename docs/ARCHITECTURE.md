...
Supabase now backs the product: authentication (sign-in/up, password reset, OAuth via `src/features/auth`), a role/RLS schema for student/parent/teacher/admin (`supabase/migrations`, [Data model and roles](DATA_MODEL_AND_ROLES.md)), and server-authoritative exam sessions. Signed-in students hit `/api/exam/session` and `/api/exam/session/:id/submit` (`src/app/api/exam`), which run selection and scoring against a server-only question bank (`src/server/exam-bank.ts`, never imported by client code) and persist sessions and attempts to `exam_sessions`/`exam_attempts`. `AssessmentScoringService` is the seam this occupies: `ServerAuthoritativeScoringService` for signed-in sessions, `LocalPracticeScoringService` unchanged for guests — see [Assessment security model](ASSESSMENT_SECURITY_MODEL.md) for the full boundary and its addendum recording this decision.

Backend code sits behind service or repository adapters that return data conforming to the existing schemas. Server-only code owns credentials, authorisation, persistence, and privileged scoring or answer-key access; no API keys or secrets belong in browser code. This boundary is what let server-authoritative scoring land as a seam swap in `exam-store.ts` rather than a rewrite of pages or renderers, and is the same boundary further backend capability — autosaved in-progress attempts, assignment workflows, reporting, content workflows — will occupy. Payments and AI-generated content are not yet built; see [Privacy and billing guardrails](PRIVACY_AND_BILLING_GUARDRAILS.md) for guardrails written ahead of the payments phase.

**Not yet persisted:** answers picked *during* an in-progress attempt live only in client state (see State management, above) — a mid-attempt refresh still loses unsaved responses, even though the session itself and the final submitted result are durable. Autosaving in-progress answers is planned; see "Recommended Phase 4 scope" in [Phase 3 hardening](PHASE3_HARDENING.md#recommended-phase-4-scope).
