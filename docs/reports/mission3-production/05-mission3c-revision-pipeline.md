...
- [ ] **(Superseded twice — see the two remediation notes at the top of this document.)** ~~`revision_blueprint_mismatch` is emitted only for the identity-binding case (§7d); no duplicate content-vs-blueprint-constraint checker exists outside structural validation.~~ First correction: `revision_blueprint_mismatch` is emitted for both the identity-binding case (wrong `parentBlueprintHash`) and the content-compatibility case (right hash, revised content's year level/subject/exam style/skill/question type incompatible with the parent's bound blueprint, §7d addendum). **Second correction:** every one of those five dimensions is mandatory and always evaluated — missing, `null`, wrongly-typed, empty, unknown, or unresolvable values are never silently skipped, only ever flagged as a mismatch. The compatibility check remains narrow and pure, never a wholesale duplicate of structural validation's full schema/registry/taxonomy logic.
