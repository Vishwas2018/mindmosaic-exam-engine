...
Status: **IMPLEMENTED — Mission 3C P1 finding remediated three times, pending independent Codex re-audit.** This document remains the authoritative design record; see `docs/reports/mission3-production/07-mission3c-revision-pipeline-delivery.md` for what was actually built, including the one necessary file-placement deviation from §6/§16 (the pipeline-runner module lives in a new sibling `pipeline/` directory, not inside `workflow/`, to avoid a real circular import) and full validation results.

**Mission 3C P1 remediation (post-audit correction — supersedes §7d and the corresponding §21 checklist line below as originally written).** An independent Codex audit found that `revision_blueprint_mismatch`, as originally scoped in §7d, checked only that `parentBlueprintHash` equalled the parent's *current* blueprint hash — an identity-binding check that proves the caller referenced the correct blueprint record but proves nothing about whether the *revised content itself* still conformed to that blueprint's cohort/subject/exam-style/skill/question-type. §7d's original text explicitly, and incorrectly, relied on structural validation to catch that drift *after* the child candidate had already been created and persisted — acceptable only if a late rejection carried no cost, which it does not: the finding is that revision-boundary protection must reject the request **before persistence**. The fix adds a pure `checkRevisionBlueprintCompatibility` function (`revision/blueprint-compatibility.ts`) that compares the proposed revised content against the parent's bound blueprint's immutable dimensions — year level/cohort, subject, exam style, skill (resolved through the same taxonomy registry `checkTaxonomy` uses), and question type — and is consulted by `ingestRevision` immediately after the `parentBlueprintHash` equality check and before the revision-limit, material-change, author-identity, or claim-resolution checks, i.e. strictly before either the parent-claim `update()` or the child `create()`. `revision_blueprint_mismatch` remains the single issue code for both the identity-binding case (wrong hash) and the content-compatibility case (right hash, incompatible content) — no new issue code was needed. See `docs/reports/mission3-production/07-mission3c-revision-pipeline-delivery.md` §4a for the corrected ten-step sequence and the full remediation record.

**Mission 3C second P1 remediation (post-audit correction — supersedes the first remediation's §7d addendum below as originally written).** A second independent Codex audit found that the first remediation's `checkRevisionBlueprintCompatibility` only compared a dimension **when the revised content already carried a correctly-typed, non-empty value for it** — a missing, `undefined`, `null`, wrongly-typed, empty, or unresolvable value caused that dimension's check to be silently *skipped* rather than flagged, letting malformed revised content claim the parent's `supersededBy` slot and create a child candidate regardless. The fix strengthens every one of the five dimension checks so each is now mandatory and always evaluated: absent, `null`, wrongly-typed, empty/whitespace-only, unknown, or unresolvable values all produce a `revision_blueprint_mismatch` mismatch entry, categorised by a `reason` (`missing` | `invalid_type` | `invalid_value` | `incompatible`) so the rejection message can distinguish "not declared" from "declared but unusable" from "a real value, just the wrong one." No new issue code was introduced. See `07-mission3c-revision-pipeline-delivery.md` §4b for the full remediation record, the corrected §7d addendum text, and the precedence tests proving malformed content is still refused before it can reach replay, request-conflict, or parent-conflict resolution.

**Mission 3C third P1 remediation (post-audit correction — the *stored* blueprint side of the same boundary, distinct from the first two remediations' *caller-declared* side).** A third independent Codex audit found that `ingestRevision`'s bound-blueprint lookup (`repository.read("blueprints", provenance.blueprintId)`) computed `blueprintHash` and ran the compatibility check only conditionally — `if (blueprintRecord !== undefined) { blueprintHash = hashJson(blueprintRecord); }`, and the compatibility check itself guarded by `parsedBlueprint?.success`. A missing, unreadable, empty, malformed-JSON, wrong-top-level-type, schema-invalid, or semantically-broken (unresolvable skill, unrenderable question type) stored blueprint record caused **both** checks to be silently skipped — never flagged — letting the revision proceed straight to claim resolution and child creation with no verified blueprint identity or immutable-field validation whatsoever. The fix replaces the conditional lookup with an explicit, typed `resolveBoundBlueprint` function (`revision/revise.ts`) that always returns either a fully-resolved, hashable `Blueprint` or a typed rejection (`revision_blueprint_missing` for an absent/unreadable/unparseable record, `revision_blueprint_invalid` for one that parses but fails `blueprintSchema` or declares an unresolvable skill/unrenderable question type) — evaluated before the hash comparison, before the compatibility check, and strictly before any parent claim or child write. Truthy/optional checks (`if (blueprintRecord)`, `parsedBlueprint?.success`) were removed entirely. See `07-mission3c-revision-pipeline-delivery.md` §4c for the full remediation record, the corrected contract text, and the adjacent-bypass audit findings this remediation deliberately did not extend into `correctness/`/`review/` (out of this task's scope).
...
**Superseded by the Mission 3C P1 remediation (see the note at the top of this document).** This subsection is retained verbatim below for the historical record of what was originally decided and why an independent Codex audit later found it insufficient; it does **not** describe current behaviour. Current behaviour: §7d below plus a mandatory **content-compatibility** check, both emitting `revision_blueprint_mismatch`, both evaluated before any mutation. See `07-mission3c-revision-pipeline-delivery.md` §4a for the corrected sequence.

**Original text (now partially superseded).** Emitted at step 3 of `ingestRevision` (§7a): when the revise request's declared `parentBlueprintHash` does not equal `hashJson` of the parent's **current** bound blueprint. This was described as an **identity-binding** check only — "is the revision still targeting the exact blueprint its parent's review findings were written against" — not a deep content-vs-constraint check.

**What this code was originally claimed *not* to need to cover (this claim is the part the audit rejected):** whether the *revised content itself* (its declared cohort/subject/exam style/skill, marks, question type, etc.) actually still conforms to that blueprint's constraints. The original reasoning was that **structural validation already checks this** (`validation/taxonomy-checks.ts`, `registry-checks.ts`, `checkAgainstProductionSchema`) once the revised candidate re-enters at `generated` and runs the complete pipeline again. **This reasoning is incorrect as a governance boundary**: it allows an incompatible child candidate to be created and a parent to be claimed (`supersededBy` stamped) *before* the later structural-validation gate has any chance to reject it — the exact "structural validation might reject the child later, but that is not acceptable" failure mode the remediation closes. The corrected design adds a narrow, pure pre-creation check (`checkRevisionBlueprintCompatibility`, §7d addendum below) that duplicates none of structural validation's full schema/registry logic — it only compares the five blueprint-bound dimensions against the one specific parent blueprint record, never the wider taxonomy-supported set structural validation checks against.

**Outcome class:** **rejection (request failure)**, never quarantine, for both the identity-binding case and the content-compatibility case. Quarantine is reserved for "the gate cannot decide about an *existing* candidate record" — this is a **pre-creation input-validation refusal**: no candidate record exists yet to quarantine. **Fail-closed, before any mutation** — `revision_blueprint_mismatch` (both variants) is checked before the revision-limit check, before the material-change check, before the parent's `supersededBy` is even consulted, and strictly before either the parent-claim `update()` or the child `create()` — exactly the "prefer fail-closed rejection before creating a new revision record" requirement.

**7d addendum — content-compatibility check (Mission 3C P1 remediation, corrected by the second P1 remediation below).** `checkRevisionBlueprintCompatibility(revisedContent, blueprint)` (`revision/blueprint-compatibility.ts`) is a pure, side-effect-free function comparing the revision's declared `yearLevel` (numeric, converted from the blueprint's `year-3`/`year-5` form), `metadata.subject`, `examStyle`, `metadata.skill` (resolved through `skillTaxonomyRegistry.resolve` — the same alias-resolution `checkTaxonomy` uses, so a skill declared via a valid alias of the blueprint's own skill is not flagged as drift), and `type` (question type) against the parent's bound `Blueprint` record's corresponding fields.

**Corrected (second P1 remediation): every one of the five dimensions is mandatory and always evaluated — there is no code path that skips a dimension.** A revision that omits a dimension, declares it `null`, declares it with the wrong runtime type, declares an empty/whitespace-only or unresolvable value, or declares a well-formed but different value is flagged identically: as a mismatch, categorised by `reason` (`missing` | `invalid_type` | `invalid_value` | `incompatible`) so the message can say *why* — e.g. `yearLevel: expected comparable year 5, received missing`, `subject: expected "reading", received invalid type number`, `skill: expected registered compatible skill "lit.reading.inference", received unresolved "totally.unknown.skill.xyz"`. (The original text here — "a field absent or not of the expected primitive type is skipped, not flagged" — described the exact defect the second remediation closed; retained struck through for the historical record: ~~a field absent or not of the expected primitive type on the revised content is skipped, not flagged — a shape defect is structural validation's job, not this gate's~~.) Any mismatch causes `ingestRevision` to reject with `revision_blueprint_mismatch`, evaluated immediately after the `parentBlueprintHash` equality check (§7a step 3) and before every check that follows it — including revision-limit, material-change, author-identity, replay, and both conflict checks, so malformed content can never reach or exploit those later checks.

**7d second addendum — bound-blueprint resolution (Mission 3C third P1 remediation).** The two addenda above both assumed the *stored* bound blueprint record itself was already valid and readable. It was not always: `provenance.blueprintId` is only ever a reference, and `repository.read("blueprints", blueprintId)` can return `undefined` (missing, unreadable, or malformed-JSON-and-quarantined-at-the-storage-layer) or a defined-but-invalid value. Prior to this remediation, both the hash check and the `checkRevisionBlueprintCompatibility` call above were gated on `blueprintRecord !== undefined`/`parsedBlueprint?.success` — an *optional* pattern that silently skipped both checks for exactly these cases, allowing a revision against a candidate whose bound blueprint had been deleted, corrupted, or never written correctly to proceed straight to claim resolution and child creation. `resolveBoundBlueprint` (`revision/revise.ts`) replaces this with an explicit, exhaustive result: either a validated `Blueprint` plus its canonical hash, or a typed rejection — `revision_blueprint_missing` (the record does not exist, or was unreadable/malformed and the storage layer already normalises that to "absent") or `revision_blueprint_invalid` (a record exists but fails `blueprintSchema`, or declares a `skill` that does not resolve against `skillTaxonomyRegistry` or a `questionType` with no registered renderer — deliberately *not* the full `validateBlueprint` planning-time validator's broader curation-quality checks, which would reject blueprints that are valid, bound, and already governing real candidates). This resolution runs before the `parentBlueprintHash` comparison and before `checkRevisionBlueprintCompatibility`, and therefore before revision-limit, material-change, author-identity, replay, and both conflict checks — a corrupted or missing blueprint is refused even for what would otherwise be a byte-identical replay of an already-accepted request (a deliberate precedence decision: the stored blueprint is re-verified on every call, never trusted merely because an earlier call once found it valid). See `07-mission3c-revision-pipeline-delivery.md` §4c for the full record, including the adjacent-module audit this remediation deliberately did not extend into.
...
- [ ] **(Superseded three times — see the three remediation notes at the top of this document.)** ~~`revision_blueprint_mismatch` is emitted only for the identity-binding case (§7d); no duplicate content-vs-blueprint-constraint checker exists outside structural validation.~~ First correction: `revision_blueprint_mismatch` is emitted for both the identity-binding case (wrong `parentBlueprintHash`) and the content-compatibility case (right hash, revised content's year level/subject/exam style/skill/question type incompatible with the parent's bound blueprint, §7d addendum). **Second correction:** every one of those five dimensions is mandatory and always evaluated — missing, `null`, wrongly-typed, empty, unknown, or unresolvable values are never silently skipped, only ever flagged as a mismatch. **Third correction:** the *stored* bound blueprint itself is now resolved via an explicit, exhaustive `resolveBoundBlueprint` — a missing, unreadable, schema-invalid, or semantically-broken (unresolvable skill/unrenderable question type) stored blueprint is refused (`revision_blueprint_missing`/`revision_blueprint_invalid`) before the hash comparison or compatibility check ever run, never silently skipping both. The compatibility check remains narrow and pure, never a wholesale duplicate of structural validation's full schema/registry/taxonomy logic.
