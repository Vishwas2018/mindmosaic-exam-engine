...
**7d addendum — content-compatibility check (Mission 3C P1 remediation).** `checkRevisionBlueprintCompatibility(revisedContent, blueprint)` (`revision/blueprint-compatibility.ts`) is a pure, side-effect-free function comparing the revision's declared `yearLevel` (numeric, converted from the blueprint's `year-3`/`year-5` form), `metadata.subject`, `examStyle`, `metadata.skill` (resolved through `skillTaxonomyRegistry.resolve` — the same alias-resolution `checkTaxonomy` uses, so a skill declared via a valid alias of the blueprint's own skill is not flagged as drift), and `type` (question type) against the parent's bound `Blueprint` record's corresponding fields. A field absent or not of the expected primitive type on the revised content is skipped, not flagged — a shape defect is structural validation's job, not this gate's. Any field present and differing produces a `RevisionBlueprintMismatch` entry; one or more entries causes `ingestRevision` to reject with `revision_blueprint_mismatch`, evaluated immediately after the `parentBlueprintHash` equality check (§7a step 3) and before every check that follows it.
...
- [ ] **(Superseded by the Mission 3C P1 remediation — see the note at the top of this document.)** ~~`revision_blueprint_mismatch` is emitted only for the identity-binding case (§7d); no duplicate content-vs-blueprint-constraint checker exists outside structural validation.~~ Corrected checklist item: `revision_blueprint_mismatch` is emitted for both the identity-binding case (wrong `parentBlueprintHash`) and the content-compatibility case (right hash, revised content's year level/subject/exam style/skill/question type incompatible with the parent's bound blueprint, §7d addendum); the compatibility check is narrow and pure, never a wholesale duplicate of structural validation's full schema/registry/taxonomy logic.
