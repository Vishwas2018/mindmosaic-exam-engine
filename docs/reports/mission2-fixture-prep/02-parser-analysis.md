...
| `difficulty: "easy" \| "medium" \| "hard" \| "challenge"` | `metadata.difficulty: "easy" \| "medium" \| "challenging"` | **Value mismatch**: harvest's `"hard"`/`"challenge"` vs trusted's single `"challenging"`. 0 occurrences of harvest `"hard"` in the corpus (only `easy`/`medium`/`challenge` observed); `"challenge"` (ICAS-only, 52 questions) must map to `"challenging"` |
