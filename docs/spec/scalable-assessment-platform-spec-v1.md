...
### 1.2 Version 1.2 amendment (2026-08-12, Phase 2 step 2b)

Version 1.1 permitted exactly one runtime reader of `item_answer_versions`: a
`SECURITY DEFINER` scoring function returning outcomes rather than rows. That
required scoring logic to exist in PL/pgSQL as a second implementation of
`src/features/exam-engine/scoring/question-scorers.ts`, which remains needed for
the guest path. Two implementations of one scoring rule is a correctness risk the
product cannot carry: a divergence between them is a learner scored differently
depending on which path ran, and no test suite makes two implementations equal —
it only reports when they have stopped being so.

This revision therefore permits a second runtime path — a **dedicated
least-privilege scoring role** confined to one isolated module — and makes its
compensating controls normative:

- §9.3 now admits two runtime answer-read paths and adds §9.3.1, the controls.
- §17.1 restates the trust boundary as the role and the module, not the function
  signature.
- §22 adds two proof obligations: the role's complete grant set, and the raw
  answer never leaving the module.

What does **not** change: `anon` and `authenticated` hold zero privileges on
`item_answer_versions`; no answer-read RPC or view is granted to `authenticated`;
...
**Runtime answer access (amended v1.2 — see §9.3.1).** The runtime reader of
`item_answer_versions` MUST be exactly one of:

1. a narrowly scoped `SECURITY DEFINER` scoring function with a fixed `search_path`
   that authorizes the caller, resolves only the answer versions allocated to that
   caller's session, and returns only scored outcomes—not answer rows; **or**
2. a **dedicated least-privilege scoring role** held by a single isolated scoring
   module, under the compensating controls in §9.3.1.

Publication/administration jobs may write through separately audited server
credentials, but general application server code MUST NOT select answer rows
directly. No general answer-read RPC or view may be granted to `authenticated`.

#### 9.3.1 Dedicated scoring role (amendment, 2026-08-12)

Where option 2 is chosen, **all** of the following are normative:

- The scoring role MUST be a distinct Postgres role. It MUST NOT be `anon`,
  `authenticated`, or `service_role`, MUST NOT hold `BYPASSRLS`, and MUST NOT be a
  member of any role that does.
- Its privileges MUST be exactly: `SELECT` on `item_answer_versions`, the minimum
  reads required to resolve which answer versions belong to the session being
  scored, and the minimum writes required to persist derived outcomes. It MUST hold
  nothing else on any object.
...
- The raw answer key, grading rules, rubric, and private explanation MUST NOT leave
  that module: never returned to a caller, never placed in a DTO, never logged
  (§17.4). Only derived outcomes—correctness, marks, manual-review status—may cross
  the boundary.
- `anon` and `authenticated` MUST retain zero privileges on `item_answer_versions`,
  unchanged.
- The role's complete grant set MUST be asserted by an automated least-privilege
  test that fails on any additional grant, and the module boundary MUST be enforced
  by an automated check rather than by convention.

The rationale, the trade this accepts, and what it forecloses are recorded in
ADR-006 Amendment A.
...
- Private answer data MUST be readable at runtime only through one of the two
  paths §9.3 permits: a narrowly scoped `SECURITY DEFINER` scoring function, or a
  dedicated least-privilege scoring role confined to a single isolated module under
  the §9.3.1 controls. Under the second path the trust boundary is the **role and
  the module**, not the function signature: the raw answer MUST NOT cross that
  module boundary in any direction, and general application server code MUST remain
  unable to read answer rows with the credentials it holds.
...
| The raw answer never leaves the scoring module | §9.3.1 module boundary | DTO contract scan, log scan, and an import-boundary check on the scoring credential |
